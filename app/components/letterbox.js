...
import {
  Box, Typography, keyframes, styled,
} from '@mui/material';
 ...
 *
 */
const StyledBox = styled(Box, { shouldForwardProp: (prop) => prop !== 'pulse' || prop !== 'value' || prop !== 'colour' || prop !== 'shake' })(({
  pulse, value, colour, shake,
}) => ({
  display: 'flex',
...
export default function Letterbox({
  letter, shakeLetter, wordIndex, letterIndex, revealed, shake, gameOver,
}) {
  ...
  useEffect(() => {
    const timeoutid = setTimeout(() => {
      if (revealed) {
        const newColour = getColour(letter.letterState);
        setColour(newColour);
      }
    }, 600 * letterIndex);
